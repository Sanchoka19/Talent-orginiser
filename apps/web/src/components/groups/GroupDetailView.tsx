...
const DICT = {
    ka: {
        ...
        slotCycleCustom: 'მორგებული... (Custom)',
...
export const GroupDetailView: React.FC = () => {
    ...
    const { groups, talents, deleteGroup, updateGroup, schedule, venues, getGroupFairnessScore } = useApp();
    ...
    // Inventory, Task & Show detail modal states
    const [selectedInventoryForDetail, setSelectedInventoryForDetail] = useState<InventoryRequirement | null>(null);
    const [selectedTaskForDetail, setSelectedTaskForDetail] = useState<InventoryRequirement | null>(null);
    const [selectedShowForDetail, setSelectedShowForDetail] = useState<ShowEvent | null>(null);

    // Talent drawer and edit state
    ...
    const members = talents.filter((tItem) => memberTalentIds.includes(tItem.id));
    ...
    const groupFairnessScore = getGroupFairnessScore(currentGroup.id);
    ...
    const handleSaveSpecialTask = (taskName: string, slots: DutySlot[], performerIds?: string[] | string) => {
        ...
        const newSpecialDutyTask: SpecialDutyTask = {
            ...
            customRotationValue: validSlots[0]?.customRotationValue,
        ...
        const newSlotRequirements: InventoryRequirement[] = validSlots.map((s, idx) => ({
            ...
            rotationCycle: s.rotationCycle || 'every_show',
            customRotationValue: s.customRotationValue,
    ...
    const handleDeleteTask = (taskId: string, parentTaskId?: string, e?: React.MouseEvent) => {
        e?.stopPropagation();
        const taskToDelete = allReqs.find((req) => req.id === taskId);
        const taskName = taskToDelete?.itemName || (isKa ? 'დავალება' : 'Task');

        confirm({
            title: isKa ? 'დავალების წაშლა' : 'Delete Task',
            message: isKa
                ? `ნამდვილად გსურთ „${taskName}“-ის წაშლა? მოვალეობა ამოიშლება ჯგუფის წესებიდან.`
                : `Are you sure you want to delete "${taskName}"? This duty will be removed from group rules.`,
            itemName: taskName,
    ...
    // Inventory delete handler with confirmation modal
    const handleDeleteInventoryItem = (itemId: string, e?: React.MouseEvent) => {
        e?.stopPropagation();
        const itemToDelete = allReqs.find((req) => req.id === itemId);
        const itemName = itemToDelete?.itemName || (isKa ? 'ინვენტარი' : 'Inventory item');

        confirm({
            title: isKa ? 'ინვენტარის წაშლა' : 'Delete Inventory Item',
            message: isKa
                ? `ნამდვილად გსურთ „${itemName}“-ის წაშლა? მორიგეობის ეს წესი ამოიშლება ჯგუფიდან.`
                : `Are you sure you want to delete "${itemName}"? This duty rule will be removed from the group.`,
            itemName: itemName,
            confirmLabel: isKa ? 'წაშლა' : 'Delete',
            variant: 'danger',
            onConfirm: () => {
                ...
                toast.success(
                    isKa
                        ? `„${itemName}“ წარმატებით წაიშალა`
                        : dict.inventoryDeletedSuccess
                );
                if (selectedInventoryForDetail?.id === itemId) {
    ...
    const getCycleLabel = (
        cycle?: TaskRotationCycle,
        customVal?: number,
        customUnit?: any
    ) => {
        switch (cycle) {
            case 'custom': {
                const val = customVal || 2;
                if (isKa) {
                    if (customUnit === 'week') return `ყოველ ${val} კვირაში`;
                    if (customUnit === 'day') return `ყოველ ${val} დღეში`;
                    return `ყოველ ${val} შოუზე`;
                } else {
                    const u = customUnit === 'week' ? 'week(s)' : customUnit === 'day' ? 'day(s)' : 'show(s)';
                    return `Every ${val} ${u}`;
                }
            }
            case 'every_show':
    ...
    const tabs: { id: GroupDetailTab; label: string; icon: React.FC<{ size?: number; className?: string }>; count: number }[] = [
        ...
        { id: 'inventory', label: dict.tabInventory, icon: Boxes, count: inventoryReqs.length },
        { id: 'tasks', label: dict.tabTasks, icon: ListTodo, count: specialTasks.length },
        ...
        <div className="w-full pb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <StatCard
                    ...
                    subtitle={`Fairness: ${groupFairnessScore}%`}
                    subtitleColor={groupFairnessScore >= 85 ? 'text-emerald-600 dark:text-emerald-400 font-semibold' : groupFairnessScore >= 60 ? 'text-amber-500 font-semibold' : 'text-danger font-semibold'}
            ...
            <div>
                {activeTab === 'members' && (
                    <GroupRosterTab
                        ...
                        onSelectTalent={(talent) => setSelectedTalentForDrawer(talent)}
                        dict={dict}
                        isKa={isKa}
                    />
                )}
            ...
            {/* Inventory Duty Detail & Assignment / Swap Modal */}
            <InventoryAssignmentModal
                isOpen={Boolean(selectedInventoryForDetail)}
                onClose={() => setSelectedInventoryForDetail(null)}
                inventoryReq={
                    selectedInventoryForDetail
                        ? (currentGroup.inventoryRequirements || []).find((r) => r.id === selectedInventoryForDetail.id) || selectedInventoryForDetail
                        : null
                }
                currentGroup={currentGroup}
            />

            {/* Special Task Detail & Assignment / Swap Drawer */}
            <TaskDetailDrawer
                isOpen={Boolean(selectedTaskForDetail)}
                onClose={() => setSelectedTaskForDetail(null)}
                task={
                    selectedTaskForDetail
                        ? (currentGroup.inventoryRequirements || []).find((r) => r.id === selectedTaskForDetail.id) || selectedTaskForDetail
                        : null
                }
                currentGroup={currentGroup}
                getCycleLabel={getCycleLabel}
            ...
            {/* Show Event Detail Modal / Side Drawer */}
            <EventDetailModal
                isOpen={Boolean(selectedShowForDetail)}
                onClose={() => setSelectedShowForDetail(null)}
                event={
                    selectedShowForDetail
                        ? schedule.find((s) => s.id === selectedShowForDetail.id) || selectedShowForDetail
                        : null
                }
            />

            {/* Talent Detail Drawer (Opens on card click) */}
            <TalentDetailDrawer
                talent={
                    selectedTalentForDrawer
                        ? talents.find((t) => t.id === selectedTalentForDrawer.id) || selectedTalentForDrawer
                        : null
                }
            ...
            {/* Talent Add/Edit Form Modal */}
            <TalentFormModal
                isOpen={isTalentFormOpen}
                onClose={() => {
