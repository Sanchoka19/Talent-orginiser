...
type FormSlot = Omit<DutySlot, 'headcount' | 'customRotationValue'> & {
  headcount: number | '';
...
export const TaskAssignmentModal: React.FC<TaskAssignmentModalProps> = ({
  ...
  // Active performers for automatic rotation pool (filtered by active status and slot gender requirement if specified)
  const slotGenderRequirement = slots[0]?.assignedGender || 'Any';

  const activeMembersForAutoRotation = useMemo(() => {
    return members.filter((m) => {
      // Must be Active
      if (m.status !== 'Active') return false;

      // Filter by slot gender if not 'Any'
      if (slotGenderRequirement === 'Male Only' && m.gender !== 'Male') return false;
      if (slotGenderRequirement === 'Female Only' && m.gender !== 'Female') return false;

      return true;
    });
  }, [members, slotGenderRequirement]);
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-5 scroll-smooth transition-all duration-300 ease-in-out">
                  ...
                  {slots.map((slot, sIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {slot.rotationCycle === 'custom' && (
                          <div className="mt-2.5 p-2 sm:px-3 sm:py-2 rounded-lg bg-purple-500/5 border border-purple-500/20 flex items-center gap-2 flex-wrap text-xs text-text-primary animate-in fade-in slide-in-from-top-1 duration-150">
                            ...
                            <input
                              ...
                              min={1}
                              max={99}
                              value={slot.customRotationValue ?? 2}
                              onChange={(e) =>
                                handleUpdateSlot(slot.id, {
                                  customRotationValue:
                                    e.target.value === ''
                              ...
                              onBlur={() => {
                                if (
                                  slot.customRotationValue === '' ||
                                  Number(slot.customRotationValue) < 1
                                ) {
                                  handleUpdateSlot(slot.id, { customRotationValue: 2 });
                                }
                              }}
                              className="w-14 px-2 py-1 text-center font-bold rounded-md border border-border-medium bg-surface text-text-primary outline-none focus:border-purple-600 focus:ring-1 focus:ring-purple-600/20"
                              placeholder="2"
                              required
                            />

                            <select
                              value={slot.customRotationUnit || 'show'}
                              onChange={(e) =>
                                handleUpdateSlot(slot.id, {
                                  customRotationUnit: e.target.value as 'show' | 'day' | 'week'
                                })
                              }
                              className="text-xs font-semibold px-2.5 py-1 rounded-md border border-border-medium bg-surface text-text-primary outline-none focus:border-purple-600 cursor-pointer"
                            >
                              <option value="show">{isKa ? 'შოუზე' : 'Show(s)'}</option>
                              <option value="day">{isKa ? 'დღეში' : 'Day(s)'}</option>
                              <option value="week">{isKa ? 'კვირაში' : 'Week(s)'}</option>
                            </select>

                            {isKa && slot.customRotationUnit !== 'show' && (
                              <span className="text-text-secondary font-semibold">ერთხელ</span>
                            )}

                            <span className="ml-auto text-[11px] text-text-tertiary hidden sm:inline-block font-normal">
                              {isKa
                                ? `(როტაცია: ყოველ ${slot.customRotationValue || 2} ${slot.customRotationUnit === 'week'
                                  ? 'კვირაში ერთხელ'
                                  : slot.customRotationUnit === 'day'
                                    ? 'დღეში ერთხელ'
                                    : 'შოუზე'
                                })`
                                : `(Rotates every ${slot.customRotationValue || 2} ${slot.customRotationUnit || 'show'})`}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            <div className="p-3.5 rounded-xl border border-slate-200 dark:border-border-subtle bg-slate-50/70 dark:bg-surface-secondary/40 flex flex-col gap-3 transition-all duration-300 ease-in-out">
              ...
              {/* Auto-Rotation Active Members Pool Preview */}
              {assignmentMode === 'auto' && (
                <div className="pt-3 border-t border-border-subtle flex flex-col gap-2.5 animate-in fade-in duration-200">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-text-primary">
                        {isKa ? 'ავტომატური როტაციის შემსრულებლები' : 'Auto-Rotation Performers'}
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                        {isKa
                          ? `${activeMembersForAutoRotation.length} აქტიური წევრი`
                          : `${activeMembersForAutoRotation.length} active members`}
                      </span>
                    </div>

                    {slotGenderRequirement !== 'Any' && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-purple-500/10 text-purple-600 dark:text-purple-300 border border-purple-500/20">
                        {isKa
                          ? (slotGenderRequirement === 'Male Only' ? 'გაფილტრულია: მხოლოდ კაცები' : 'გაფილტრულია: მხოლოდ ქალები')
                          : `Filtered: ${slotGenderRequirement}`}
                      </span>
                    )}
                  </div>

                  <p className="text-[11px] text-text-secondary m-0 leading-relaxed">
                    {isKa
                      ? slotGenderRequirement !== 'Any'
                        ? `სლოტის მოთხოვნის მიხედვით როტაციაში მონაწილეობენ მხოლოდ ${slotGenderRequirement === 'Male Only' ? 'კაცი' : 'ქალი'} აქტიური შემსრულებლები.`
                        : 'სისტემა დავალებას სამართლიანად გადაანაწილებს დასის ყველა აქტიურ წევრზე.'
                      : 'System will rotate this task among all eligible active members.'}
                  </p>

                  {/* Active Members Grid */}
                  {activeMembersForAutoRotation.length === 0 ? (
                    <div className="p-4 text-center rounded-lg border border-dashed border-border-medium bg-surface text-xs text-text-secondary">
                      {isKa
                        ? 'მითითებული პარამეტრებით აქტიური შემსრულებლები ვერ მოიძებნა'
                        : 'No active members match the specified criteria'}
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
                      {activeMembersForAutoRotation.map((member) => (
                        <div
                          key={member.id}
                          className="flex items-center gap-2.5 p-2 rounded-lg border border-border-subtle bg-surface hover:border-border-medium transition-all"
                        >
                          <div className="relative shrink-0">
                            <img
                              src={getTalentAvatar(member)}
                              alt={member.firstName}
                              className="w-8 h-8 rounded-full object-cover border border-purple-500/30 shadow-2xs"
                            />
                            <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-2 ring-surface" title={isKa ? 'აქტიური' : 'Active'} />
                          </div>
                          ...
                          <span className="text-[9px] px-1.5 py-0.5 rounded font-medium bg-surface-secondary text-text-secondary shrink-0">
                            {member.gender === 'Male' ? (isKa ? 'კაცი' : 'M') : (isKa ? 'ქალი' : 'F')}
                          </span>
                        </div>
                      ))}
                    </div>
